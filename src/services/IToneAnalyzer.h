...
#ifndef SELF_ITONEANALYZER_H
#define SELF_ITONEANALYZER_H

#include "utils/IService.h"
#include "SelfLib.h"

struct DocumentTones;

//! Wrapper for getting the tone of some text
class SELF_API IToneAnalyzer : public IService
{
public:
	RTTI_DECL();
	...
	//! Construction
	IToneAnalyzer( const std::string & a_ServiceId, AuthType a_AuthType = AUTH_BASIC ) : 
		IService( a_ServiceId, a_AuthType )
	{}

	virtual void GetTone( const std::string & a_Text, OnMessage a_Callback ) = 0;
};

struct SELF_API Tone : public ISerializable
{
	double m_Score;
	std::string m_ToneId;
...
struct SELF_API ToneCategory : public ISerializable
{
...
struct SELF_API DocumentTones : public ISerializable
{
	RTTI_DECL();
